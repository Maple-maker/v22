...
def extract_items_gcss_standard(tables: List[List[List[str]]]) -> List[BomItem]:
    ...
    for table in tables:
        ...
        for row_num, row in enumerate(table[1:]):
            ...
            # Extract description - handle various multi-line formats
            desc_cell = row[indices['description']] if indices['description'] < len(row) else None
            description = ""
            if desc_cell:
                # The description cell can have multiple formats:
                # 1. "SHORT,NAME\nFULL DESCRIPTION (DETAILS)" - prefer line with spaces/parens
                # 2. "NAME,WRAPPED,ACROSS\nMULTIPLE\nLINES" - combine all lines
                # 3. "TRUNCATED/\nFRAG\nCLEAN VERSION" - prefer the clean version
                
                raw_text = str(desc_cell).strip()
                lines = raw_text.split('\n')
                
                # Strategy: Find the best single line OR combine if needed
                best_line = ""
                combined = ""
                ...
                for line in lines:
                    ...
                    # Build combined version (skip fragments like "DES" or "COMPONENTS")
                    ...
                    # Look for the best single line:
                    # - Has spaces (readable format like "SCREEN, CAMOUFLAGE")
                    # - Has parentheses (has details)
                    # - Doesn't start with "(" (that's a continuation)
                    # - Is longer than current best
                    has_spaces = ', ' in line or ' ' in line
                    has_parens = '(' in line and ')' in line
                    starts_with_paren = line.startswith('(')
                    
                    if has_spaces and not starts_with_paren:
                        if has_parens and (not best_line or '(' not in best_line):
                            best_line = line
                        elif len(line) > len(best_line) and not best_line:
                            best_line = line
                
                # Use best line if found, otherwise use combined text
                if best_line and len(best_line) >= 10:
